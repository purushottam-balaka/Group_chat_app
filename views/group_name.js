async function createGroup(e){
    try{        
        ...
        const res= await axios.post('http://localhost:9000/req_users',grpName,{headers:{'Authorization':token}})
        console.log('res',res)
...
function showUser(element,g_id){
    ...
    element.forEach(ele => {
        ...
        addBut.onclick=async()=>{
            ...
            const token=localStorage.getItem('token')
            await axios.post('http://localhost:9000/add_user',details,{headers:{'Authorization':token}})
            .then(()=>{
